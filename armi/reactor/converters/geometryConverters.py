...
class BlockNumberModifier(GeometryChanger):
    ...
    def setNumberOfBlocks(self, assem, refineIndex=[]):
        ...
        """
        activeBlocks = [assem[iBlock] for iBlock in refineIndex]
        activeHeight = sum([b.getHeight() for b in activeBlocks])
        numActive = len(activeBlocks)
        newBlockStack = []
        blockList = assem.getBlocks()
        inertLower = blockList[: refineIndex[0]]
        inertUpper = blockList[refineIndex[-1] + 1 :]

        newBlockStack = inertLower
